...
dic = f['dic']
nam =f['nam']
inv = DBTable("Invoice Table")
l = [('ID No',str),('Name',str),('Invoice Nos',int),('Phone No',int),('Invoice Date',str),
     ('Due',float),('Address',str),('Email',str)]
inv.add_fieldlist(l)
for i in nam.keys():
    inv[i] = nam[i]


log = DBTable("Product Table")
l = [('name',str),('category' ,str),('description' ,str),('id',str),
     ('itemnr' ,int),('Cost Price',float),('Price',float),('quantity',int)]
log.add_fieldlist(l)
import time as t

b4 = t.time()
for i in dic.keys():
    key = log.generateuniquekey("I")
    log[key] = dic[i]
print t.time()-b4
    
count =0
log.set_fieldtype('quantity',int)
log.set_fieldtype('id',str)
for i in log:
    log[i]["id"] = str(count)
    count += 1
log.add_field("Lists",list)
